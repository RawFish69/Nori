...
from lib.config import RAID_ITEM_TIERS, RAID_NAMES, RAID_WEB_URL, format_aspect_display, format_compacted_item_lines, format_ward_display
from lib.raid_pool_utils import _normalize_gambit_loot_shape, current_gambit_refresh_interval, load_aspect_lootpool, load_gambit_pool, load_weekly_raid_pool
from lib.views.raid import aspectView, raidItemView
loader = lightbulb.Loader()
SECONDS_PER_WEEK = 604800
raid = lightbulb.Group('raid', 'Raid lootpool command group')

@raid.register
class ShowRaidGambits(lightbulb.SlashCommand, name='gambit', description='View daily raid gambits'):
    ...
    async def invoke(self, ctx: lightbulb.Context) -> None:
        ...
        if not data:
            await ctx.edit_response(-1, content='Gambit data is not available yet - the background refresh has not produced a file yet. Please check back shortly.')
            return
        ...
        gambit_embed = hikari.Embed(title='Daily Raid Gambit Rotation', description=description, color='#c0aaff')
        ...
        if rotation_expired:
            ...
            gambit_embed.add_field('Rotation Refreshing', f'The current rotation window has ended. A fresh refresh should land within {max(1, next_interval // 60)} minutes.\nIf entries still do not change after this window, data may be outdated - please remind a maintainer to verify it.')
        elif gambits_all:
            lines = []
            ...
            gambit_embed.add_field('No Gambits Yet', 'No gambit data has been reported for this rotation yet.')
        gambit_embed.set_footer('Nori Bot - Raid Gambits')
        gambit_embed.set_thumbnail('https://nori.fish/resources/aspect.gif')
        await ctx.edit_response(-1, embed=gambit_embed, content='')

@raid.register
class ShowRaidAspectLootpool(lightbulb.SlashCommand, name='aspect', description='View weekly raid aspect lootpool'):
    ...
    async def invoke(self, ctx: lightbulb.Context) -> None:
        ...
        for raid_name in RAID_NAMES:
            ...
            lines = [f"- {format_aspect_display(aspect_name, aspect_icons.get(aspect_name, ''))}" for aspect_name in mythic_names]
        ...
        message = await ctx.edit_response(-1, embed=loot_embed, content='', components=view.build())
        ctx.client.app.d.miru.start_view(view, bind_to=message)
        await view.wait()

@raid.register
class ShowRaidItemLootpool(lightbulb.SlashCommand, name='item', description='View weekly raid item lootpool'):
    ...
    async def invoke(self, ctx: lightbulb.Context) -> None:
        ...
        for raid_name in RAID_NAMES:
            ...
            if isinstance(raid_data, dict) and any((isinstance(raid_data.get(tier), list) and raid_data.get(tier) for tier in RAID_ITEM_TIERS)):
                any_data = True
        if not any_data:
            loot_embed.add_field('Raid Item Pool Not Ready', 'No raid item pool has been published yet. Please check back shortly.')
            await ctx.edit_response(-1, embed=loot_embed, content='')
            return
