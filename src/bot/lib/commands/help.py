...
@loader.command
class NoriCmd(lightbulb.SlashCommand, name='nori', description='Nori bot information'):

    @lightbulb.invoke
    async def invoke(self, ctx: lightbulb.Context) -> None:
        uptime_str = get_uptime(config.deploy_time)
        restart_ts = int(config.deploy_time)
        nori_embed = hikari.Embed(title='Multi-purpose Utility Bot', color='#FFFFFF')
        nori_embed.add_field('Status', f'**Online**\nMode: **{MODE}**\nLast restarted: <t:{restart_ts}:R>\nUptime: {uptime_str}', inline=False)
        nori_embed.add_field('Links', f'[Nori-Web]({NORI_API_BASE_URL})\n[Support server](https://discord.gg/tU7eaKAWb2)\n[Add Nori to your server](https://discord.com/application-directory/873677970928193568)\n[GitHub](https://github.com/RawFish69/Nori)', inline=False)
        ...
        try:
            nori_user = await ctx.client.app.rest.fetch_user(873677970928193568)
...
@loader.command
class HelpCmd(lightbulb.SlashCommand, name='help', description="Shows this bot's help menu."):
