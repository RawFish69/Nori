"""
Description: 5x5 gameboard logic for tictactoe (chat interaction mini game)
Author: RawFish
"""
import hikari
import miru
import functools
import random
from nori-bot import mini_game, game_record


class GameViewFive(miru.View):
    def __init__(self, *args, **kwargs):
        # Create gameboard for 5x5 space (2D)
    ...
    def take_side(self, board, username):
        sides = [i for i in range(25) if i not in [0, 4, 20, 24, 12]]  # no corners and center
        for side in sides:
            if board[side] == ' ':
                return side
        return None
