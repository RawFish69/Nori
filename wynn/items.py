def item_log():
    with open("/app/bot/item_change.log", 'r') as log:
        changelog = log.read()
    print(changelog)
    return changelog
...
def item_amp(name, tier):
    ...
    if found == False:
        ...
        if 'attackSpeed' in result:
            display += f"Attack Speed: {result.get('attackSpeed')}\n"
        display += item_sp
        display += 'Amplifier Applied: {:.2f}% Increase to IDs\n'.format(amp * 100)
        stat_display = ''
        for stat in result:
            value = result.get(stat)
            if stat in sp_req:
                ...
                if int(value) > 0:
                    ...
                    star = ''
                    if 'spellcost' not in stat.lower():
                        if 1.0 <= amp_roll < 1.25:
                            star = '*'
                        elif 1.25 <= amp_roll < 1.3:
                            star = '**'
                        elif amp_roll == 1.3:
                            star = '***'
                    max_val = round(value * 1.3)
                    if 'spellcost' in stat.lower():
                        ...
                        id_rolled = round(value * amp_roll)
                        min_val = round(value * 0.3)
                        percentage = ((id_rolled - min_val) / (max_val - min_val)) * 100
                    overall += percentage
                    stat_count += 1
        ...
        if 'majorIds' in result:
            ...
            display += '\n'
            display += 'No major ID'
        display += f'\nRerolled with Amp {tier}\n'
    return display, reroll_stats
