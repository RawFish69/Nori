...
# delete = ['set', 'restrictions', 'addedLore', 'mainAttackElementalDamageBonus', 'spellElementalDamageBonus', 'spellNeutralDamageBonusRaw',
#           'mainAttackElementalDamageBonusRaw', 'elementalDamageBonusRaw', "spellElementalDamageBonusRaw"]


def wim_item_map():
    with open('wynn_items.json') as f:
    ...
    for d in data:
        ...
        for key, value in d.items():
            if key in wim_mappings:
                new_key = wim_mappings[key]
    ...
    #     new_dict = {}
    #     for key, value in d.items():
    #         if value is not None and key not in delete:
    #             new_dict[key] = value
    #         new_dict[key] = value
    #     if new_dict:
    #         new_data.append(new_dict)

    data_wim = {"data": new_data}
    data_wim.update({"version": "2.0.3"})

    with open('Equipments.json', 'w') as f:
        f.write(json.dumps(data_wim, indent=2))
    print(f"{len(data)} items processed.")
...
def wb_item_map():
    ...
    for i in data:
        ...
        for key, value in i.items():
            if key in wb_mappings:
                new_key = wb_mappings[key]
                wb_dict[new_key] = value
            else:
                wb_dict[key] = value
        i.clear()
        i.update(wb_dict)
    ...
    data_wb = {"items": wb_data}

    with open('items.json', 'w') as file:
        file.write(json.dumps(data_wb))
    print(f"{len(data)} items processed.")
